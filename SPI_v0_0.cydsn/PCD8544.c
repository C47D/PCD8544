...
uint8_t PCD8544_UpdateXmin = 0, PCD8544_UpdateXmax = 0, PCD8544_UpdateYmin = 0, PCD8544_UpdateYmax = 0;
uint8_t PCD8544_x;
uint8_t PCD8544_y;
...
void PCD8544_Send(uint8_t data){
    ...
    //SS_N5110_Write(0);
    #if CY_PSOC5
    while(!(SPI_Nokia5110_ReadTxStatus() & SPI_Nokia5110_STS_SPI_IDLE));
    SPI_Nokia5110_WriteTxData(data);
    while(!(SPI_Nokia5110_ReadTxStatus() & SPI_Nokia5110_STS_SPI_IDLE));
    #elif CY_PSOC4
    /* TODO */
    SPI_Nokia5110_SpiUartWriteTxData((uint32) txData);
    #endif
    /* CE High */
    //SS_N5110_Write(1);
}
...
void PCD8544_Delay(uint16_t micros){
    CyDelayUs(micros);
}
...
void PCD8544_DrawPixel(uint8_t x, uint8_t y, PCD8544_Pixel_t pixel) {
	if (x >= PCD8544_WIDTH) {
		return;
	}
	if (y >= PCD8544_HEIGHT) {
...
void PCD8544_UpdateArea(uint8_t xMin, uint8_t yMin, uint8_t xMax, uint8_t yMax) {
...
void PCD8544_Putc(char c, PCD8544_Pixel_t color, PCD8544_FontSize_t size) {
	uint8_t c_height, c_width, i, b = 0, j;
...
void PCD8544_DrawLine(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, PCD8544_Pixel_t color) {
	short dx, dy;
	short temp;
...
void PCD8544_DrawRectangle(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, PCD8544_Pixel_t color) {
...
void PCD8544_DrawFilledRectangle(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, PCD8544_Pixel_t color) {
	for (; y0 < y1; y0++) {
